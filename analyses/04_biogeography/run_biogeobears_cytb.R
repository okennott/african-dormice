############################################################
# BioGeoBEARS automated pipeline
# 
...
TREE_FILE      <- "bitimetree_cytb.nwk"
GEOG_FILE      <- "geography_file_CYTB.txt"       # <-- PHYLIP format
OUTPUT_DIR     <- "Output_CYTB/"      # <-- output folder
# MAX_RANGE_SIZE <- 4                           # or use the next line's
MAX_RANGE_SIZE <- ncol(getranges_from_LagrangePHYLIP(lgdata_fn=GEOG_FILE)@df)-3 # Max Ranges minus the non-African ranges
N_CORES        <- max(1, parallel::detectCores() - 4)  # use all but one core
